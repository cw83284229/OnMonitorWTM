  ...
  public  class DaHuaSDKcs
    {
        ...
        /// <param name="ChannelID"></param>
        /// <returns></returns>
        public DVRChannelInfo GetEncodeConfig(IntPtr _LoginID, int ChannelID)
        {
           DVRChannelInfo  channelInfo  = new DVRChannelInfo();
            ...
            channelInfo.Number = ChannelID;
            channelInfo.ChannelName = GetChannelname(_LoginID,ChannelID);
            channelInfo.Resolution = (_VideoInfo.nWidth * _VideoInfo.nHeight).ToString();
            channelInfo.nHeight = _VideoInfo.nHeight;
            channelInfo.nWidth = _VideoInfo.nWidth;
            channelInfo.nFrameRate = _VideoInfo.nFrameRate;
            channelInfo.nIFrameInterval = _VideoInfo.nIFrameInterval;
            channelInfo.nBitRate = _VideoInfo.nBitRate;
            channelInfo.emBitRateControl = _VideoInfo.emBitRateControl.ToString();
            channelInfo.emCompression = _VideoInfo.emCompression.ToString();
        ...
        ///// 开启监控画面预览
        ///// </summary>
        ///// <param name="m__LoginID"></param>
        ///// <param name="Channel_ID"></param>
        ///// <param name="PlayHandle"></param>
        ///// <returns></returns>
        //public IntPtr RealPlay(IntPtr m__LoginID, int Channel_ID,IntPtr PlayHandle)
        //{
        //    IntPtr m_PlayID = IntPtr.Zero;
        //    if (IntPtr.Zero == m__LoginID)
        //    {
        //         m_PlayID = NETClient.RealPlay(m__LoginID, Channel_ID, PlayHandle);
        ...
        ///// 停止预览
        ///// </summary>
        ///// <param name="m_PlayID"></param>
        ///// <returns></returns>
        //public bool StopRealPlay(IntPtr m_PlayID)
        //{
        //  bool requst=  NETClient.StopRealPlay(m_PlayID);
        ...
        /// <param name="ChannelID"></param>
        /// <returns></returns>
        public string GetChannelPictureLocal(IntPtr m_LoginID, int ChannelID)
        {
            string path = AppDomain.CurrentDomain.BaseDirectory + "image";
            if (!Directory.Exists(path))
            {
        ...
        //    PictureBox pic = new PictureBox();
            IntPtr m_RealPlayID =NETClient.RealPlay(m_LoginID, ChannelID, IntPtr.Zero, EM_RealPlayType.Realplay);//开始预览
            Thread.Sleep(500);
            bool ret = NETClient.CapturePicture(m_RealPlayID, filePath, EM_NET_CAPTURE_FORMATS.JPEG);//开始抓图
                                                                                                     // Thread.Sleep(500);
            StopRealplay(m_RealPlayID);
            Thread.Sleep(500);
            if (!ret)
            {
               // MessageBox.Show(NETClient.GetLastError());
