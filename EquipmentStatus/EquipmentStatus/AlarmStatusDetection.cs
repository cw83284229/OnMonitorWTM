...
namespace EquipmentStatus
{
    public class AlarmStatusDetection
    {
        ...
        internal static readonly string MonitorTypeKey = ConfigurationManager.AppSettings["MonitorType"];//获取配置文件信息
        ...
        public static void TaskLoginStartListen()
        {
            ...
            var listAlarmHost =http.GetAlarmHosts().Where(x => x.MonitorRoom.RoomType.Contains(MonitorTypeKey));
            //  service.HashDelete("AlarmStatusData", "data");取消redis
        ...
        public static async Task LoginStartListenAsync(Guid id, string ip, string prot, string user, string pwd)
        {
           ...
           // var alarmdata = http.GetAlarmsByHostIP(ip);
            ...
            if (IntPtr.Zero != lLoginID)
            {
                ...
                if (result)
                {
                    ...
                    loghelper.WriteLog($"IP：{ip}开启监听模式,时间:{DateTime.Now.ToString()}");
                }
            ...
            else
            {
                ...
                loghelper.WriteLog($"IP：{ip}登陆失败, 时间:{DateTime.Now.ToString()}");
            }
        }
        ...
        private static bool AlarmCallBackEx(int lCommand, IntPtr lLoginID, IntPtr pBuf, uint dwBufLen, IntPtr pchDVRIP, int nDVRPort, bool bAlarmAckFlag, int nEventID, IntPtr dwUser)
        {
           ...
           // HttpHelper http = new HttpHelper();
            var alarmdata = http.GetAlarmsByHostIP(hostIp);
            switch (type)
            {
                        ...
                        if (info.nAction == 0)  // 0:开始 1:停止(设备内0表示报警)
                        {
                            if (alarmstate != null)
                            {
                                ...
                                if (alarmstate.FirstOrDefault().Value == "ARMING")//报警
                                {
                                    ...
                                    AppAlarmManage.AlarmId =alarm.ID;
                                    ...
                                    http.AddAlarmManage(AppAlarmManage);    
                ...
                case EM_ALARM_TYPE.ALARM_DEFENCE_ARMMODE_CHANGE://撤布防状态变化写入
                    {
                        ...
                        var alarmstatuslist = alarmdata.Where(u => u.AlarmHost.AlarmHostIP == hostIp);
                        ...
                        #endregion   //更新AlarmManage数据库数据

                        AlarmManage appAlarmManage = http.GetAlarmManage(alarm.Alarm_ID).OrderByDescending(u => u.UpdateTime).FirstOrDefault();
                        ...
                        if (appAlarmManage != null && appAlarmManage.AlarmTime != null)
                        {
                            ...
                            {   //判断异常
                                ...
                                appAlarmManage.UpdateTime = DateTime.Now;
                                http.UpdateAlarmManage(appAlarmManage);
                            }
                            else
                            {

                                AlarmManage appAlarmManage3 = new AlarmManage();
                                appAlarmManage3.AlarmId = alarm.ID;
                                ...
                                else if (info.emDefenceStatus == EM_DEFENCEMODE.DISARMING)
                                {
                                    ...
                                    http.AddAlarmManage(appAlarmManage3);
                                }
                            }
